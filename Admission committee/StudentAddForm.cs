namespace AdmissionCommittee
{
    public partial class StudentAddForm : Form
    {
        ...
        private void save_Click(object? sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Выберите форму обучения!", "Ошибка валидации",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
            ...
            int mathScore = (int)nudMathScores.Value;
            int rusScore = (int)nudRusScores.Value;
            int csScore = (int)nudComputerScienceScores.Value;
            int totalScore = mathScore + rusScore + csScore;

            NewStudent = new Student
            {
                FullName = txtFullName.Text,
                Gender = cmbGender.SelectedItem.ToString() ?? "М",
                DateBirth = dtpDateBirth.Value,
                FormOfEducation = cmbFormOfEducation.SelectedItem.ToString() ?? "Очная",
